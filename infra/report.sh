...

{
  ...
  echo "  \"github\": \"$( {
    ...
    if [ -z "$t" ]; then printf 'токена нет'; else
      ...
      printf 'длина=%s ' "${#t}"
      R=$(curl -s -m 15 -H "Authorization: Bearer $t" -w '\n%{http_code}' \
  ...
  # Проверяем именно рабочий публичный путь. IPv6 временно удалён из DNS:
  # отчёт показывал 000 для сайта и панели, и часть мобильных сетей выбирала
  # этот мёртвый адрес вместо исправного IPv4.
  echo "  \"сайты_ipv4\": \"jobtoo=$(curl -4 -s -o /dev/null -m 8 -w '%{http_code}' https://jobtoo.ru/ 2>/dev/null || echo нет) / admin=$(curl -4 -s -o /dev/null -m 8 -w '%{http_code}' https://admin.jobtoo.ru/ 2>/dev/null || echo нет)\","
