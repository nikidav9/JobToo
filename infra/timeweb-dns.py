...
# Список нарочно хранится в одном месте: две разошедшиеся зоны — это способ
# однажды переключиться и не понять, почему половина перестала работать.
#
...
# IPv6 на машине есть, но внешний HTTPS по нему не отвечает: серверный
# отчёт 01.09 показывал `ipv6_сайт=000 / admin=000`, тогда как IPv4 и все
# внутренние службы были живы. Из-за AAAA мобильные сети сначала выбирали
# неработающий путь, поэтому сайт и вход в дашборд то открывались, то висели.
# Эти типы раньше управлялись этим скриптом и теперь должны быть удалены, а
# не просто перестать добавляться.
RETIRED_RECORD_TYPES = {'AAAA'}
...
def diff(sub: str):
    ...
    наши_типы = {r[0].upper() for r in WANT[sub]} | RETIRED_RECORD_TYPES
